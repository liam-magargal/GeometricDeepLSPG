...
def segment(N, Nc, edge_index, pos):
    ...
    # get adjacency matrix and graph Laplacian
    ...
    pos2[:,0] = (pos2[:,0] - lBound*torch.ones((Nc))) / (rBound - lBound)
    ...
    edge_index2 = radius_graph(x=pos2,r=torch.sqrt(torch.tensor(9./torch.pi/Nc)),loop=False)
...
# generate edge index of first graph
edge_index = radius_graph(x=pos,r=torch.sqrt(torch.tensor(9./torch.pi/n1)),loop=False)
