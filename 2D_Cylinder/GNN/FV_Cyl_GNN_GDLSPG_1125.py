...
def saveOutputIter(cell_centroids, f, output_directory, filename, N, iter_out):
    cell_centroids = cell_centroids.flatten(order='F')
    
    data_out = np.concatenate((cell_centroids, f), axis=None)
    data_out = np.reshape(data_out, (N,6), order='F')
    
    df = pd.DataFrame(data_out)
    
    if t<10:
        name = output_directory + filename + "000000" + str(iter_out) + ".csv"
    elif t<100:
        name = output_directory + filename + "00000" + str(iter_out) + ".csv"
    elif t<1000:
        name = output_directory + filename + "0000" + str(iter_out) + ".csv"
    elif t<10000:
        name = output_directory + filename + "000" + str(iter_out) + ".csv"
    elif t<100000:
        name = output_directory + filename + "00" + str(iter_out) + ".csv"
    elif t<1000000:
        name = output_directory + filename + "0" + str(iter_out) + ".csv"
    else:
        name = output_directory + filename + str(iter_out) + ".csv"
...
if __name__=="__main__":
    ...
    model = torch.load('lat' + str(lat_dim)).cpu()
    ...
    labels5 = torch.load(directory + 'labels5').to(device)
    ...
    f = np.zeros((4*N))
    lat_hist = torch.zeros((1001,lat_dim)) 
    ...
    for t_step in range(1001):
        ...
        # decode to approximate high-dimensional representation
