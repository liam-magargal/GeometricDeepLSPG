...
@njit(nb.types.Tuple((float64,float64,float64,float64,float64,float64))(float64,float64,float64,float64,float64,float64))
def getConserved_S( rho, u, v, P, gamma, vol ):
    Mass   = rho
    Mom_x  = rho * u
    Mom_y  = rho * v
    E = 1/(gamma-1)*P/rho + .5*(u*u + v*v)
...
@njit(nb.types.Tuple((float64,float64,float64,float64,float64,float64))(float64,float64,float64,float64,float64,float64))
def getPrimitive_S( Mass, Mom_x, Mom_y, Energy, gamma, vol ):
    rho = Mass
    u  = Mom_x / rho
    v  = Mom_y / rho
    E = Energy / rho
...
if __name__=="__main__":
    ...
    output_directory       = 'Outputs/mach_'+str(sys.argv[1])+'/'
    filename               = 'output'
    N                      = np.shape(cell_centroids)[0]
    gamma                  = 1.4
    courant_fac            = 0.4
    ...
    # current time step values (t=0)
    rho_c, vx_c, vy_c, P_c = genDomain(cell_centroids,gamma,u_1)
    Mass_c, Momx_c, Momy_c, Energy_c, E_c, H_c = getConserved( rho_c, vx_c, vy_c, P_c, gamma, cell_volumes )
    
    # next time step values (initialize)
    rho_n, vx_n, vy_n, P_n = genDomain(cell_centroids,gamma,u_1)
